...
export const ROUND_INFOS: RoundInfo[] = [
  {
    ...
    tag: "온라인",
    prompt:
      '한국 초등 3-4학년 가족오락관 스피드 퀴즈용 단어 80개를 JSON 배열로 줘. 3분 게임에서 패스가 많아도 반복이 적게 나오도록 충분히 다양하게 골라. 2025-2026년 한국 초등학생이 실제로 말하거나 보는 트렌드를 반영해. 너무 쉬운 단일 일상명사(예: 사과, 강아지, 피자, 공, 책, 물)는 전체 80개 중 8개 이하만 허용하고, 대부분은 설명하면 알아듣지만 살짝 웃기거나 요즘 느낌 나는 단어로 골라. 카테고리는 게임/앱 15개 이상(예: 로블록스, 마인크래프트, 브롤스타즈, 오비, 브루크헤이븐처럼 초등학생에게 익숙한 것), 숏폼/밈/유행어 10개 이상(예: 스키비디, 브레인롯, 챌린지, 쇼츠처럼 가족이 웃으며 설명 가능한 것), 캐릭터/굿즈/팬문화 15개 이상(예: 포켓몬카드, 산리오, 쿠로미, 티니핑, 라부부처럼 초등·키즈 문화권 단어), 먹거리/놀이/학교생활 20개 이상(예: 마라탕, 탕후루, 요아정, 랜덤깡, 학원숙제, 급식메뉴처럼 한국 현실감 있는 것), 가족/풀빌라 상황 10개 이상을 섞어. 난이도는 쉬움 15개, 보통 45개, 도전 20개 정도로 섞고, 한 라운드 안에서 비슷한 단어가 겹치지 않게 해. 배열 순서도 매번 무작위로 섞어. 너무 성인용, 정치/선정/혐오/공포가 강한 단어, 설명하기 민망한 단어, 부모가 전혀 설명하기 어려운 과도한 은어는 빼. 형식: {"words": ["단어1", "단어2", ...]}',
  },
  {
    type: "blur_image",
    icon: "🌫️",
